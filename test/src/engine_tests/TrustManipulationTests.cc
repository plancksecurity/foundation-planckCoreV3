...
void TrustManipulationTests::check_trust_manipulation() {
    ...
    TEST_ASSERT(user_again->fpr);

    char* keypair2 = strdup(user_again->fpr);
    cout << "generated fingerprint \n";
    cout << user_again->fpr << "\n";

    TEST_ASSERT(strcmp(user->fpr, user_again->fpr) != 0);
    update_identity(session, user);
    TEST_ASSERT(strcmp(user->fpr, keypair1) == 0);
    ...
    TEST_ASSERT(user->comm_type == PEP_ct_OpenPGP);
    TEST_ASSERT(strcmp(user->fpr, keypair2) == 0);
    ...
    TEST_ASSERT(strcmp(user->fpr, keypair2) == 0);
    TEST_ASSERT(user->comm_type == PEP_ct_OpenPGP_unconfirmed);
    ...
    status = get_trust(session, user);
    TEST_ASSERT(strcmp(user->fpr, keypair2) == 0);
    TEST_ASSERT(user->comm_type == PEP_ct_mistrusted);
    ...
    status = update_identity(session, user);
    TEST_ASSERT(user->fpr == NULL);
    TEST_ASSERT(user->comm_type == PEP_ct_key_not_found);
