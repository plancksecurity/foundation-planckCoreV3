...
void ReencryptPlusExtraKeysTests::check_reencrypt_plus_extra_keys() {
    ...
    TEST_ASSERT_MSG(status == PEP_KEY_IMPORTED, "Failed to import own recipient public key.");
    status = import_key(session, own_recip_priv_key.c_str(), own_recip_priv_key.length(), NULL);
    TEST_ASSERT_MSG(status == PEP_KEY_IMPORTED, "Failed to import own recipient private key.");    
    status = import_key(session, own_recip_2_pub_key.c_str(), own_recip_2_pub_key.length(), NULL);
    TEST_ASSERT_MSG(status == PEP_KEY_IMPORTED, "Failed to import own second recipient public key.");
    ...
    TEST_ASSERT_MSG(status == PEP_KEY_IMPORTED, "Failed to import own sender public key.");
    status = import_key(session, recip_2_pub_key.c_str(), recip_2_pub_key.length(), NULL);
    TEST_ASSERT_MSG(status == PEP_KEY_IMPORTED, "Failed to second recipient public key.");
    status = import_key(session, recip_0_pub_key.c_str(), recip_0_pub_key.length(), NULL);
    TEST_ASSERT_MSG(status == PEP_KEY_IMPORTED, "Failed to import zeroth recipient public key.");
    status = import_key(session, pub_extra_key_0.c_str(), pub_extra_key_0.length(), NULL);
    TEST_ASSERT_MSG(status == PEP_KEY_IMPORTED, "Failed to import first extra public key.");
    status = import_key(session, pub_extra_key_1.c_str(), pub_extra_key_1.length(), NULL);
    TEST_ASSERT_MSG(status == PEP_KEY_IMPORTED, "Failed to import second extra public key.");
