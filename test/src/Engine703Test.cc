...
TEST_F(Engine703Test, check_engine703_valid_import_renew) {
    ...
    ASSERT_TRUE(slurp_and_import_key(session,"test_keys/short_future.asc"));    

    pEp_identity* me = new_identity("future@expires.soon", "A04E2786845B8CB622239E5EF11C452A91A55A07", PEP_OWN_USERID, "Future Renew");
    PEP_STATUS status = set_own_key(session, me, "A04E2786845B8CB622239E5EF11C452A91A55A07");
    ASSERT_EQ(status, PEP_STATUS_OK);
    status = myself(session, me);
    ASSERT_EQ(status, PEP_STATUS_OK);    
    ASSERT_STRNE(me->fpr, nullptr);  
    ASSERT_STREQ(me->fpr, "A04E2786845B8CB622239E5EF11C452A91A55A07");  
    char* keydata = NULL;
    size_t size = 0;
    status = export_secret_key(session, me->fpr, &keydata, &size);
    
    ofstream outfile;
    outfile.open("703_short_future_extended.asc");
    outfile << keydata;
    outfile.close();
...
// TEST_F(Engine703Test, check_engine703_generated) {
// 
//     // (pEp_identity) $1 = {
//     //   address = 0x000000010160a460 "test010@exchange.peptest.ch"
//     //   fpr = 0x000000010160a8c0 "D4B3091B5BC8BA5DF4A0BFD900187F11F2216F5F"
