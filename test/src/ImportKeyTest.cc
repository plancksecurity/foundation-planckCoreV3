...
TEST_F(ImportKeyTest, check_import_fpr_pub_new) {
    ...
    status = import_key_with_fpr_return(session, pubkey.c_str(), pubkey.size(), NULL, NULL, &keylist, NULL);
...
TEST_F(ImportKeyTest, check_import_change_pub_new) {
    ...
    stringlist_t* keylist = NULL;
    uint64_t changes = 0;
    status = import_key_with_fpr_return(session, pubkey.c_str(), pubkey.size(), NULL, NULL, &keylist, &changes);
    ASSERT_EQ(status, PEP_KEY_IMPORTED);
    ASSERT_NOTNULL(keylist);
    ASSERT_STREQ(keylist->value, "4ABE3AAF59AC32CFE4F86500A9411D176FF00E97");
    ASSERT_NULL(keylist->next);
...
TEST_F(ImportKeyTest, check_import_change_pub_nochange_binary_bigkey) {
    ...
    stringlist_t* keylist = NULL;
    uint64_t changes = 0;
    status = import_key_with_fpr_return(session, img, img_size, NULL, NULL, &keylist, &changes);
    ASSERT_EQ(status, PEP_KEY_IMPORTED);
...
TEST_F(ImportKeyTest, check_import_fpr_list_pub_concat) {
    ...
    stringlist_t* keylist = NULL;
    uint64_t changes = 0;
    PEP_STATUS status = import_key_with_fpr_return(session, pubkey_material.c_str(), pubkey_material.size(), NULL, NULL, &keylist, &changes);
    ASSERT_EQ(status, PEP_KEY_IMPORTED);
    ASSERT_NOTNULL(keylist);    
...
TEST_F(ImportKeyTest, check_import_fpr_list_priv_concat) {
    ...
    stringlist_t* keylist = NULL;
    uint64_t changes = 0;
    PEP_STATUS status = import_key_with_fpr_return(session, privkey_material.c_str(), privkey_material.size(), NULL, NULL, &keylist, &changes);
    ASSERT_EQ(status, PEP_KEY_IMPORTED);
    ASSERT_NOTNULL(keylist);    
    ASSERT_EQ(stringlist_length(keylist), 10);
...
TEST_F(ImportKeyTest, check_import_fpr_list_priv_then_pub) {
    ...
    changes = 0;
    string pubkey_material = slurp("test_keys/pub/import_keys_multi_pub_concat.asc");
    status = import_key_with_fpr_return(session, pubkey_material.c_str(), pubkey_material.size(), NULL, NULL, &keylist, &changes);
    ASSERT_EQ(status, PEP_KEY_IMPORTED);
    ASSERT_NOTNULL(keylist);    
    ASSERT_EQ(stringlist_length(keylist), 10);
...
TEST_F(ImportKeyTest, check_import_fpr_list_pub_then_priv) {
    ...
    changes = 0;    
    string privkey_material = slurp("test_keys/priv/import_keys_multi_priv_concat.asc");
    status = import_key_with_fpr_return(session, privkey_material.c_str(), privkey_material.size(), NULL, NULL, &keylist, &changes);
    ASSERT_EQ(status, PEP_KEY_IMPORTED);
    ASSERT_NOTNULL(keylist);    
    ASSERT_EQ(stringlist_length(keylist), 10);
    // ASSERT_EQ(changes, 1023);  // The answer to this might be implementation dependent and we don't care.
...
TEST_F(ImportKeyTest, check_import_huge_concat_then_change) {
    ...
    status = import_key_with_fpr_return(session, some_changed_material.c_str(), some_changed_material.size(), NULL, NULL, &keylist, &changes);
    ASSERT_EQ(status, PEP_KEY_IMPORTED);
    ASSERT_NOTNULL(keylist);    
    ASSERT_EQ(stringlist_length(keylist), 10);
