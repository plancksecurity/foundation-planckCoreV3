...
static void work(PEP_SESSION session, bool own, kind_of_reset kind,
    ...
    std::string userid
        = (own ? PEP_OWN_USERID : "some-non-own-userid");
    ...
    const char *key_fpr = "906C9B8349954E82C5623C3C8C541BD4E203586C";
    pEp_identity *identity = NULL;
    status = set_up_ident_from_scratch(session, key_file,
    ...
    if (! own) {
        /* In Engine 2.x the expected_status at run number 1 was, at first,
           PEP_ILLEGAL_VALUE -- that was questionable and error-prone, so it was
           changed into PEP_KEY_NOT_FOUND in 2.1.69 (2023-01-27).  In Engine 3.x
           this is PEP_STATUS_OK , which is not compatible but perfectly
           reasonable. */
    }
    for (int i = 0; i < 2; i ++) {
