    ...
    )
{
    ...
    dst->enc_format = PEP_enc_signed_only;
    
    // _src is an object that will hold the relevant message parts (sans headers)
    // for MIME-encoding. It does not duplicate the message parts, nor does
    // it take ownership of them, and should thus NOT be freed via free_message
    ...
    // Get mime-encoded string of message
    status = _mime_encode_message_internal(_src, true, &mimetext, true);
    ...
    status = sign_only(session, fpr, mimetext, strlen(mimetext),
                       &stext, &ssize);
...
DYNAMIC_API PEP_STATUS sign_message(
        PEP_SESSION session,
        message *src,
        message **dst,
        char** fpr_used
    )
{
    ...
    *fpr_used = NULL;
    ...
    // FIXME: Find out what happens when this is empty.
    msg = clone_to_empty_message(src);
    if (msg == NULL)
        goto enomem;

    attach_own_key(session, msg);
    status = sign_PGP_MIME(session, src, fpr, msg);        
