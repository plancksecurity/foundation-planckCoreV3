                                    ...
                                    bool ping)
{
    ...
    LOG_EVENT("send %s from %s <%s> to %s <%s>, status after encrypting %i %s", (ping ? "Ping" : "Pong"), from->username, from->address, to->username, to->address, status, pEp_status_to_string(status));
    if (status == PEP_STATUS_OK)
