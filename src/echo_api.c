                                            ...
                                            const pEp_identity *identity)
{
    ...
    if (status != PEP_STATUS_OK) echo_log(stderr, "make_sure_identity_exists: status is %i\n", (int) status);
    return status;
}
...
{
    assert(session && identity && below_rate_limit_p);
    if(! (session && identity && below_rate_limit_p))
        return PEP_ILLEGAL_VALUE;
    PEP_STATUS status = PEP_STATUS_OK;
    ...
    * below_rate_limit_p = false;
    ...
    reset_and_clear_bindings(session->echo_get_echo_below_rate_limit);
    ...
    ON_SQL_ERROR_SET_STATUS_AND_GOTO;
    assert(sql_status == SQLITE_ROW);
    assert(sqlite3_column_count(session->echo_get_echo_below_rate_limit) == 1);
    ...
    if (status != PEP_STATUS_OK) echo_log(stderr, "echo_get_below_rate_limit: status is %i\n", (int) status);
    return status;
}
...
{
    assert(session && identity
           /* This does not make much sense if used on own identities, but there
              is no harm in allowing that anyway. */);
    if(! (session && identity))
        return PEP_ILLEGAL_VALUE;
    PEP_STATUS status = PEP_STATUS_OK;
    int sql_status = SQLITE_OK;
    ...
    /* Bind SQL statement parameters. */
    reset_and_clear_bindings(session->echo_set_timestamp);
    ...
    ON_SQL_ERROR_SET_STATUS_AND_GOTO;
    assert(sql_status == SQLITE_DONE);

    echo_log(stderr, "set last Echo timestamp to now for %s <%s>\n", (identity->username ? identity->username : "NOUSERNAME"), (identity->address ? identity->address : "NOADDRESS"));

 end:
    if (status != PEP_STATUS_OK) echo_log(stderr, "echo_set_last_echo_timestamp: status is %i\n", (int) status);
    return status;
}
                       ...
                       const Distribution_t *pong_distribution_message)
{
    ...
    if (memcmp(actual_response, expected_response, sizeof(pEpUUID))) {
        ...
        /* Good response.  Okay, notify the application that some rating
           might have improved.  */
        fprintf(stderr, "session->notifyHandshake is %s\n", (session->notifyHandshake == NULL ? "NULL" : "non-NULL"));
                                    ...
                                    bool ping)
{
    ...
    if (status != PEP_STATUS_OK) {
        echo_log(stderr, "echo_get_below_rate_limit failed with status %i on %s <%s>\n", (int) status, (to->username ? to->username : "<no username>"), (to->address ? to->address : "<no address>"));
        return status;
    }
    if (! below_rate_limit) {
        echo_log(stderr, "rate limit exceeded: not sending a %s to %s <%s>\n", (ping ? "Ping" : "Pong"), (to->username ? to->username : "<no username>"), (to->address ? to->address : "<no address>"));
        return PEP_STATUS_OK;
    }
    ...
    echo_log(stderr, "sent %s from %s <%s> to %s <%s>\n", (ping ? "Ping" : "Pong"), from->username, from->address, to->username, to->address);
    ...
    echo_log(stderr, "echo_set_last_echo_timestap: status is %i\n", (int) status);
