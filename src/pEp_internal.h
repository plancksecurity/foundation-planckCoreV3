...
#define LOG_MESSAGE_WITH(literal_string, the_message, macro)            \
    do {                                                                \
        ...
        if (the_message == NULL)                                        \
            ...
            if (_log_message_m->from && _log_message_m->from->username) \
                _from_username = _log_message_m->from->username;        \
            if (_log_message_m->from && _log_message_m->from->address)  \
            ...
            macro(literal_string ": [%s %s, recv_by %s, %s (%s <%s> -> %s <%s>)]", \
                  ...
                  ((_log_message_m->dir == PEP_dir_incoming)            \
                   ? "incoming" : "outgoing"),                          \
                  _from_username, _from_address,                        \
                  _to_username, _to_address);                           \
        }                                                               \
    } while (false)
