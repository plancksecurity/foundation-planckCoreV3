...
static PEP_STATUS mime_encode_message_signed_only(
        const message * msg,
        bool omit_fields,
        struct mailmime **result
    )
{
    struct mailmime * mime = NULL;
    struct mailmime * submime = NULL;
    struct mailmime * subsubmime = NULL;    
	struct mailmime_parameter * param;
    int r;
    PEP_STATUS status;
    char *plaintext;
    ...
    // FIXME: get the right algorithm here
    // FIXME: param failure?
    param = mailmime_parameter_new_with_data("micalg", "pgp-sha256");
    clist_append(mime->mm_content_type->ct_parameters, param);

    param = mailmime_param_new_with_data("protocol", "application/pgp-signature");
    clist_append(mime->mm_content_type->ct_parameters, param);

    submime = part_multiple_new("multipart/mixed");
    assert(submime);
    if (submime == NULL)
        goto enomem;
    r = mailmime_smart_add_part(mime, submime);
